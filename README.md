...
- XMTP profile sync: on startup/pair/rebuild/name-change, Takobot verifies XMTP profile metadata when read APIs exist and repairs mismatches when write APIs exist; when direct profile writes are unavailable it upserts Convos-compatible profile metadata into conversation `group.appData` (`ConversationCustomMetadata` protobuf with `profiles[].inboxId/name/image`) instead of sending chat-message JSON, generates a deterministic avatar at `.tako/state/xmtp-avatar.svg`, and records sync/broadcast state in `.tako/state/xmtp-profile.json` and `.tako/state/xmtp-profile-broadcast.json`
