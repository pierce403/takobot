...
- **Properties**:
  ...
  - Supports assisted pi login in TUI via `inference login`, relaying login prompts and accepting operator replies with `inference login answer <text>`; `inference login force` bypasses stale "auth-ready" detection to force a fresh re-auth.
  ...
  - Pi sync inference retry now preserves `--print --mode text --no-session` for model-thinking fallback (`minimal`->`low`, `xhigh`->`high`) and only drops optional flags when retrying true unknown-option CLI incompatibilities.
...
- **Test Criteria**:
  ...
  - [x] Manual `explore <topic>` writes structured world notes and surfaces stage/mood-aware insight narration (no fixed hardcoded highlight phrase).
