...
- Pairing/runtime now verifies XMTP profile metadata against identity when possible, repairs mismatches when profile update APIs are available, and creates a local deterministic avatar cache (`.tako/state/xmtp-avatar.svg` + `.tako/state/xmtp-profile.json`).
