...
- XMTP profile sync: on startup/pair/rebuild/name-change, Takobot verifies XMTP profile metadata when read APIs exist and repairs mismatches when write APIs exist; it also generates a deterministic avatar at `.tako/state/xmtp-avatar.svg` and records detailed sync state in `.tako/state/xmtp-profile.json` (including when the installed XMTP SDK has no profile-metadata write API)
