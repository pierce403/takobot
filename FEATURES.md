...
- **Properties**:
  ...
  - XMTP runtime startup/rebuild and pairing/name-update flows run profile verify+repair: when profile read APIs exist, Tako checks whether name/avatar match identity and only applies updates on mismatch; when SDK profile write APIs are missing, Tako also publishes profile metadata JSON messages (`{"type":"profile",...}`) to self/known DMs (and on active DM reply paths) so compatible clients can hydrate name/avatar from chat history. Deterministic avatar is generated at `.tako/state/xmtp-avatar.svg`, and detailed sync/broadcast state is recorded at `.tako/state/xmtp-profile.json` and `.tako/state/xmtp-profile-broadcast.json`.
